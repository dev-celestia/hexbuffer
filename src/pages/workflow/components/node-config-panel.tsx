...
export function NodeConfigPanel({ node, onClose, onUpdate, onDelete, onRun }: Readonly<NodeConfigPanelProps>) {
  ...
  return (
    <div className="flex h-full w-full flex-col bg-background">
        ...
        <div className="ml-auto flex items-center gap-0.5">
          {onDelete && (
            <Button
              variant="ghost"
              size="sm"
              className="w-6 p-0 text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-500/10"
          ...
          {def.description && (
            <Tooltip>
              ...
              <TooltipContent side="bottom" sideOffset={6} className="max-w-52">
                <p className="text-2xs">{def.description}</p>
              </TooltipContent>
            </Tooltip>
          )}
        </div>
      ...
      {/* Tabbed content */}
      <Tabs defaultValue="config" className="flex-1 flex-col min-h-0">
        <TabsList className="mx-4 mt-3 h-7 w-auto shrink-0">
          <TabsTrigger value="config" className="text-2xs h-6">
            Config
          </TabsTrigger>
          {isLiveTrafficTrigger && (
            <TabsTrigger value="traffic" className="text-2xs h-6">
              Traffic
            </TabsTrigger>
          )}
          <TabsTrigger value="properties" className="text-2xs h-6">
            Properties
          </TabsTrigger>
          <TabsTrigger value="data-flow" className="text-2xs h-6">
            Data Flow
          </TabsTrigger>
        </TabsList>

        <TabsContent value="config" className="min-h-0 mt-0 data-[state=inactive]:hidden">
        ...
        {isLiveTrafficTrigger && (
          <TabsContent value="traffic" className="min-h-0 mt-0 data-[state=inactive]:hidden">
            <ScrollArea className="h-full">
        ...
        <TabsContent value="properties" className="min-h-0 mt-0 data-[state=inactive]:hidden">
          <ScrollArea className="h-full">
            <div className="px-4 py-3 space-y-3">
              <div className="space-y-1.5">
                <span className="text-3xs uppercase tracking-wider text-muted-foreground">Node ID</span>
                <p className="text-xs font-mono text-muted-foreground break-all select-all">{node.id}</p>
              </div>
              <div className="space-y-1.5">
                <span className="text-3xs uppercase tracking-wider text-muted-foreground">TextTIcon</span>
                <p className="text-xs font-mono text-muted-foreground break-all">{data.nodeType}</p>
              </div>
              <div className="space-y-1.5">
                <span className="text-3xs uppercase tracking-wider text-muted-foreground">Category</span>
                <p className="text-xs capitalize text-muted-foreground">{category}</p>
              </div>
              <div className="space-y-1.5">
                <span className="text-3xs uppercase tracking-wider text-muted-foreground">Position</span>
        ...
        <TabsContent value="data-flow" className="min-h-0 mt-0 data-[state=inactive]:hidden">
          <ScrollArea className="h-full">
            <div className="px-4 py-3">
