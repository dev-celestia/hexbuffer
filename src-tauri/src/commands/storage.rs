...
pub async fn get_storage_info(
    ...
    // Recursive directory walks (payload segments + artifact dirs) are
...
pub async fn delete_storage_artifact(
    ...
    let payload_store = payload_store.inner().clone();
    // Connection close/reopen, file deletes and DB reinit are blocking.
    tauri::async_runtime::spawn_blocking(move || {
        let app_data_dir = crate::paths::get_shared_app_dir();
...
pub async fn reset_all_app_data(
    ...
    // DB file deletes, recursive artifact cleanup and DB reinit are blocking.
    tauri::async_runtime::spawn_blocking(move || {
        // ponytail: stop proxy and terminate running browsers/crawls cleanly
        let _ = crate::proxy::stop();
        if let Some(browser_state) = app_for_task.try_state::<BrowserProcessState>() {
            let _ = stop_browser_process(&browser_state);
        }
        if let Some(ai_browser_state) = app_for_task.try_state::<AiBrowserState>() {
            stop_all_active_crawls(&app_for_task, &ai_browser_state);
        }
        ...
        if intercept_browser_profile_removed {
            fs::remove_dir_all(&intercept_browser_profile_dir)
        ...
        let mock_forge_state =
            app_for_task.try_state::<crate::commands::mock_forge::MockForgeState>();
